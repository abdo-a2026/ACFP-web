...
const App = (function() {
    ...
    function startSession(demo) {
      ...
      if (demo) {
        ...
        statusDot.innerHTML = '🔬 وضع تجريبي';
      } else {
        statusDot.className = 'status-dot';
        statusDot.innerHTML = '● متصل';
      }
    ...
    function renderBookings(searchQuery) {
      ...
      if (!data.length) {
        tbody.innerHTML = `<tr><td colspan="10"><div class="table-empty"><div class="empty-icon">📅</div><p>لا توجد حجوزات</p></div></td></tr>`;
        return;
      }
      ...
      tbody.innerHTML = data.map(b => `
        <tr>
          ...
          <td>${b.linkedPatientId ? '<span class="badge badge-green">✅ راجع</span>' : '<span class="badge badge-gray">لم يراجع</span>'}</td>
          <td>
            <div class="td-actions">
              <button class="btn-icon primary" onclick="App.openEditBooking('${b.id}')" title="تعديل">✏️</button>
              <button class="btn-icon success" onclick="App.openConvertBooking('${b.id}')" title="تحويل إلى مريض" ${b.linkedPatientId ? 'disabled' : ''}>👤</button>
              <button class="btn-icon danger" onclick="App.deleteBookingConfirm('${b.id}')" title="حذف">🗑️</button>
            </div>
          </td>
        </tr>
      `).join('');
    ...
    function renderPatients(searchQuery) {
      ...
      if (!data.length) {
        tbody.innerHTML = `<tr><td colspan="12"><div class="table-empty"><div class="empty-icon">👥</div><p>لا توجد سجلات مرضى</p></div></td></tr>`;
        return;
      }
      ...
      tbody.innerHTML = data.map(p => `
        <tr>
          ...
          <td>${p.bookingId ? '<span class="badge badge-green">🔗 من حجز</span>' : '<span class="badge badge-violet">🚶 مباشر</span>'}</td>
          <td>
            <div class="td-actions">
              <button class="btn-icon primary" onclick="App.openEditPatient('${p.id}')" title="تعديل">✏️</button>
              <button class="btn-icon danger" onclick="App.deletePatientConfirm('${p.id}')" title="حذف">🗑️</button>
            </div>
          </td>
        </tr>
      `).join('');
    ...
    function setupFormEvents() {
      ...
      // Search
      document.getElementById('search-bookings')?.addEventListener('input', e => renderBookings(e.target.value));
      ...
      if (doctorsList) {
        doctorsList.innerHTML = s.doctors.map(d => `
          <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border)">
            ...
            <button class="btn-icon danger" onclick="App.removeDoctor('${d}')">🗑️</button>
          </div>
        `).join('');
      }
      ...
      if (servicesList) {
        servicesList.innerHTML = s.services.map(svc => `
          <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border)">
            ...
            <button class="btn-icon danger" onclick="App.removeService('${svc}')">🗑️</button>
          </div>
        `).join('');
      }
    }
    ...
    function exportBookings() {
      const data = Store.applyBookingFilters(Store.getBookings(), currentFilters).map(b => ({
        'رقم الحجز': b.id, 'الاسم': b.patientName, 'الهاتف': b.phone,
        'الطبيب': b.doctorName, 'التاريخ': b.appointmentDate, 'الوقت': b.appointmentTime,
        'المصدر': b.source, 'الحالة': b.status, 'مرتبط بمريض': b.linkedPatientId ? 'نعم' : 'لا'
      }));
    ...
    function exportPatients() {
      const data = Store.applyPatientFilters(Store.getPatients(), currentFilters).map(p => ({
        'رقم المريض': p.id, 'الاسم': p.fullName, 'الهاتف': p.phone,
        'الجنس': p.gender, 'الطبيب': p.doctorName, 'الخدمة': p.serviceType,
        'السعر': p.totalPrice, 'المصاريف': p.expenses, 'صافي الربح': p.netProfit,
        'حصة الطبيب': p.doctorShare, 'حصة العيادة': p.clinicShare, 'حصة المنصة': p.platformShare,
        'من حجز': p.bookingId ? 'نعم' : 'لا'
      }));
    ...
    function exportReport() {
      ...
      const data = [
        ...
        { 'المؤشر': 'صافي الربح', 'القيمة': stats.totalNetProfit },
        { 'المؤشر': 'متوسط الربح لكل مريض', 'القيمة': stats.avgProfit },
      ];
